...
const TRACK_NAMES = { "sea-of-information": "Sea of information", "city-of-dawn": "City of dawn" } as const;
...
export function GameApp() {
  ...
  useEffect(() => {
    ...
    if (Number.isFinite(parsed)) { setVolume(parsed); audioRef.current.setVolume(parsed); }
  ...
  const persist = useCallback((next: GameState) => {
    saveGame(next); setHasSave(true); setSavedFlash(true);
    window.setTimeout(() => setSavedFlash(false), 900);
  }, []);

  const goToScene = useCallback((sceneId: SceneId) => {
    setDialogue(null); setLineIndex(0);
    setState(prev => {
      const target = scenes[sceneId]; const nextFlags = { ...prev.flags };
      ...
      const next: GameState = { ...prev, sceneId, flags: nextFlags, unlockedMusic: nextMusic, updatedAt: new Date().toISOString() };
      queueMicrotask(() => persist(next)); return next;
    });
  }, [persist]);

  const runAction = useCallback((action?: HotspotAction) => {
    if (!action) return; audioRef.current?.resume();
    if (action.type === "advance") return goToScene(action.to);
    if (action.type === "dialogue") { setDialogue(dialogues[action.dialogueId]); setLineIndex(0); return; }
    if (action.type === "setFlagAndAdvance") {
      setState(prev => { const next = { ...prev, flags: { ...prev.flags, [action.flag]: true }, updatedAt: new Date().toISOString() }; queueMicrotask(() => persist(next)); return next; });
      return goToScene(action.to);
    }
    setState(prev => { const next = { ...prev, flags: { ...prev.flags, [action.flag]: true }, updatedAt: new Date().toISOString() }; queueMicrotask(() => persist(next)); return next; });
    setDialogue(dialogues[action.dialogueId]); setLineIndex(0);
  }, [goToScene, persist]);
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => { const action = musicFocus.action; setMusicFocus(null); runAction(action); }, 1100);
    return () => window.clearTimeout(id);
  }, [musicFocus, musicPosition, runAction]);

  const openDialogue = useCallback((id: string) => { setDialogue(dialogues[id]); setLineIndex(0); }, []);
  ...
  const advanceDialogue = useCallback(() => {
    if (!dialogue) return; audioRef.current?.resume();
    if (lineIndex < dialogue.lines.length - 1) return setLineIndex(i => i + 1);
    const after = dialogue.after; setDialogue(null); setLineIndex(0); if (after) runAction(after);
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if ((event.key === "Enter" || event.key === " ") && dialogue) { event.preventDefault(); advanceDialogue(); }
    };
    window.addEventListener("keydown", onKey); return () => window.removeEventListener("keydown", onKey);
  }, [screen, dialogue, advanceDialogue]);

  const startNewGame = () => { audioRef.current?.resume(); const next = { ...INITIAL_STATE, updatedAt: new Date().toISOString() }; setState(next); persist(next); setScreen("game"); };
  const continueGame = () => { const existing = loadSave(); if (!existing) return; setState(existing); setScreen("game"); };
  const changeVolume = (next: number) => { setVolume(next); audioRef.current?.setVolume(next); window.localStorage.setItem("sea-of-information:volume", String(next)); };
  ...
  if (screen === "archive") {
    ...
    return <main className="archiveScreen"><header className="archiveHeader"><div><p className="eyebrow">MUSIC ARCHIVE</p><h2>記憶された音楽</h2></div><button onClick={() => { audioRef.current?.pause(); setScreen("title"); }}>BACK</button></header>
      <div className="archiveGrid">{(["sea-of-information", "city-of-dawn"] as const).map((id, i) => <button key={id} className="trackCard" disabled={!unlocked.includes(id)} onClick={() => audioRef.current?.play(id, true)}><span>{String(i + 1).padStart(2, "0")}</span><strong>{unlocked.includes(id) ? TRACK_NAMES[id] : "LOCKED"}</strong><small>{unlocked.includes(id) ? "Takubo29" : "—"}</small></button>)}</div>
    </main>;
  }
  ...
  return <main className={`gameScreen art-${scene.art}`} onPointerDown={() => audioRef.current?.resume()}>
    ...
    {scene.id === "vertical-slice-end" && <section className="sliceEnd"><p>VERTICAL SLICE 0.4</p><h2>明日は、まだ記録されていない。</h2><p>CHAPTER 1 — City of Dawn COMPLETE</p><div><button onClick={() => setScreen("title")}>TITLE</button><button onClick={() => { deleteSave(); setHasSave(false); startNewGame(); }}>RESTART</button></div></section>}
...
function MusicFocus({ track, position, until, label, phase }: { track: keyof typeof TRACK_META; position: number; until: number; label: string; phase: "listening" | "ready" }) {
  const meta = TRACK_META[track]; const progress = Math.min(100, Math.max(0, (position / meta.duration) * 100));
  const fmt = (value: number) => `${Math.floor(value / 60)}:${String(Math.floor(value % 60)).padStart(2, "0")}`;
  const art = track === "sea-of-information" ? "/art/sea-music.webp" : "/art/city-music.webp";
  return <section className={`musicFocus musicFocus-${phase}`}><img className="musicFocusArt" src={art} alt="" /><div className="musicFocusShade" /><div className="musicFocusContent"><p className="musicFocusKicker">{phase === "ready" ? "SCENE UNLOCKED" : "LISTENING PHASE"}</p><h2>{meta.title}</h2><p className="musicFocusMessage">{phase === "ready" ? "音が次の場面へつながりました。" : label}</p><div className="musicWave">{Array.from({ length: 48 }, (_, i) => <i key={i} style={{ height: `${18 + ((i * 17) % 52)}%` }} />)}</div><div className="musicFocusProgress"><i style={{ width: `${progress}%` }} /></div><div className="musicFocusMeta"><span>{fmt(position)}</span><span>次の場面 {fmt(until)}</span><span>{fmt(meta.duration)}</span></div><p className="musicFocusHint">{phase === "ready" ? "進行可能 — 戻ります" : "この間は操作せず、音と景色に耳を澄ませてください"}</p></div></section>;
}
...
function SceneArt({ art }: { art: string }) {
  ...
  return <div className="sceneArt" aria-hidden="true">{realArt && <img className={`sceneIllustration sceneIllustration-${art}`} src={realArt} alt="" />}{art === "sea" && <><div className="seaHorizon"/><div className="reiSilhouette"/></>}{art === "terminal" && <><div className="terminalMonolith"><span>ADMIN</span></div><div className="reiSilhouette near"/></>}{art === "dive" && <><div className="diveRing"><span>DIVE</span></div><div className="reiSilhouette"/></>}{["city","city-glitch","city-investigation"].includes(art) && <><div className="clockTower"><span>08:42</span></div><div className="childFigure"/><div className="noaFigure"/></>}{art === "noa" && <><div className="noaPortrait"><div className="scarf"/></div><div className="reiSilhouette left"/></>}{art === "aurora-gate" && <><div className="auroraTunnel"/><div className="auroraDoor"><span>AURORA</span></div></>}{art === "aurora" && <><div className="auroraCore"><span>AURORA</span></div><div className="auroraGrid"/></>}{art === "dusk" && <><div className="duskSun"/><div className="clockTower"><span>08:43</span></div></>}{art === "night" && <><div className="stars"/><div className="clockTower"><span>20:14</span></div></>}{art === "end" && <div className="endGlow"/>}</div>;
}
